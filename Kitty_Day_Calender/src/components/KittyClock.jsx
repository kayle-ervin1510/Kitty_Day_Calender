  ...
  return (
    ...
    >
      ...
      <polygon points="34,36 43,8 54,29"
        style={{ fill: 'var(--accent-2)', stroke: 'var(--text-secondary)', strokeWidth: 1.5, strokeLinejoin: 'round' }} />
      <polygon points="37,33 43,13 51,28" style={{ fill: 'var(--bg-card)' }} />
      <polygon points="76,29 87,8 96,36"
        style={{ fill: 'var(--accent-2)', stroke: 'var(--text-secondary)', strokeWidth: 1.5, strokeLinejoin: 'round' }} />
      <polygon points="79,28 87,13 93,33" style={{ fill: 'var(--bg-card)' }} />

      {/* Head — drawn after ears so it covers their bases naturally */}
      <circle cx="65" cy="50" r="28"
        style={{ fill: 'var(--accent-2)', stroke: 'var(--text-secondary)', strokeWidth: 1.5 }} />

      {/* Eyes — sleepy when collapsed, wide open when expanded */}
      {expanded ? (
        <>
          <circle cx="55" cy="47" r="7"
            style={{ fill: 'var(--bg)', stroke: 'var(--text-primary)', strokeWidth: 1 }} />
          <circle cx="55" cy="48" r="3.5" style={{ fill: 'var(--text-primary)' }} />
          <circle cx="56.5" cy="46.5" r="1.2" style={{ fill: 'white' }} />
          <circle cx="75" cy="47" r="7"
            style={{ fill: 'var(--bg)', stroke: 'var(--text-primary)', strokeWidth: 1 }} />
          <circle cx="75" cy="48" r="3.5" style={{ fill: 'var(--text-primary)' }} />
          <circle cx="76.5" cy="46.5" r="1.2" style={{ fill: 'white' }} />
        </>
      ) : (
        <>
          <ellipse cx="55" cy="47" rx="7" ry="3" style={{ fill: 'var(--text-primary)' }} />
          <ellipse cx="75" cy="47" rx="7" ry="3" style={{ fill: 'var(--text-primary)' }} />
        </>
      )}

      {/* Muzzle bump — cream snout area below nose */}
      <ellipse cx="65" cy="64" rx="11" ry="8"
        style={{ fill: 'var(--bg-card)', opacity: 0.72, stroke: 'var(--text-secondary)', strokeWidth: 0.8 }} />

      {/* Nose */}
      <polygon points="65,57 61,62 69,62" style={{ fill: 'var(--accent-3)' }} />

      {/* Mouth */}
      <path d="M61,62 Q65,68 69,62"
        style={{ fill: 'none', stroke: 'var(--text-primary)', strokeWidth: 1.2, strokeLinecap: 'round' }} />

      {/* Whiskers — extend outward from muzzle edges */}
      <line x1="29" y1="58" x2="54" y2="61"
        style={{ stroke: 'var(--text-secondary)', strokeWidth: 1.1, opacity: 0.85 }} />
      ...
      <circle cx={CX} cy={CY} r={R + 3}
        style={{ fill: 'var(--text-primary)', opacity: 0.08 }} />
      <circle cx={CX} cy={CY} r={R}
        style={{ fill: 'var(--bg-card)', stroke: 'var(--text-secondary)', strokeWidth: 2 }} />

      {/* Hour markers */}
      {markers.map((mk, i) => (
        <line key={i} x1={mk.x1} y1={mk.y1} x2={mk.x2} y2={mk.y2}
          style={{ stroke: 'var(--text-primary)', strokeWidth: mk.major ? 2 : 1, opacity: mk.major ? 0.7 : 0.35 }} />
      ))}

      {/* Cardinal numbers */}
      {cardinals.map(({ n, x, y }) => (
        <text key={n} x={x} y={y} textAnchor="middle" dominantBaseline="central"
          style={{ fill: 'var(--text-primary)', fontSize: '8px', fontWeight: 'bold', opacity: 0.75 }}>
          {n}
        </text>
      ))}
      ...
      <line x1={CX} y1={CY} x2={hourTip.x} y2={hourTip.y}
        style={{ stroke: 'var(--text-primary)', strokeWidth: 3.5, strokeLinecap: 'round' }} />
      <line x1={CX} y1={CY} x2={minuteTip.x} y2={minuteTip.y}
        style={{ stroke: 'var(--text-primary)', strokeWidth: 2, strokeLinecap: 'round' }} />
      {expanded && (
        <line x1={CX} y1={CY} x2={secondTip.x} y2={secondTip.y}
          style={{ stroke: 'var(--accent-1)', strokeWidth: 1.5, strokeLinecap: 'round' }} />
      )}
      <circle cx={CX} cy={CY} r="4" style={{ fill: 'var(--text-primary)' }} />
      {expanded && <circle cx={CX} cy={CY} r="2" style={{ fill: 'var(--accent-1)' }} />}

      {/* Digital display */}
      <rect x="17" y="171" width="96" height="27" rx="5"
        style={{ fill: 'var(--bg)', stroke: 'var(--text-secondary)', strokeWidth: 1.5 }} />
      <text x="65" y="189" textAnchor="middle" dominantBaseline="central"
        style={{ fill: 'var(--text-primary)', fontSize: '13px', fontWeight: 'bold', fontFamily: 'monospace' }}>
        {digitalTime}
      </text>
