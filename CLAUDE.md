...
| `VITE_APP_URL` | Base URL for invite links generated by `generateInvite()` — falls back to `window.location.origin` if absent |
...
| `/family/join` | `JoinFamilyPage` | Family invite acceptance — intentionally outside both layout wrappers; reads `?token=` query param and calls `acceptInvite()` |
...
| `sharedEvents` | Events from family account owners that are `family_visible = true`, fetched via the `get_shared_events_for_user` security-definer RPC |
...
| `refreshFamilyMembers()` | Re-fetches `family_members` and `sharedEvents` for the current user's family account |
...
RPCs:
- `get_email_by_username(p_username)` — returns the email for a given username (used by `login()`)
- `get_shared_events_for_user()` — security-definer function; returns `user_events` rows that are `family_visible = true` and owned by the account whose `family_members` row links to the current user via `linked_user_id`
- `accept_family_invite(p_token)` — security-definer function; marks the invite accepted and sets `family_members.linked_user_id` to the calling user's `user_profiles.id`
